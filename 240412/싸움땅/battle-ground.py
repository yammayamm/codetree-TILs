...
def fight(p1,p2): #온애가 이길 수도 있는데??
    ...
    if gun[loser]>0:
        x,y = player[loser][:2]
        heapq.heappush(board[x][y], gun[loser])
        gun[loser] = 0
    # 이동
    ...
    if board[nx][ny]:
        gun[loser] = board[nx][ny][-1]
        board[nx][ny] = board[nx][ny][:-1]

    # [2-2-3] 이긴 플레이어는 떨어져있는 총과 원래 있던 총 중 가장 공격력 높은 총 획득
    x, y, d, s = player[winner]
    if board[x][y] and gun[winner] < board[x][y][-1]:
        tmp = gun[winner]
        gun[winner] = board[x][y][-1]
        board[x][y] = board[x][y][:-1]
        heapq.heappush(board[x][y], tmp)
for _ in range(k):
    for i in range(len(player)):
        ...
        if (nx,ny) not in player_loc:
            if board[nx][ny]: # 칸에 총이 있으면 비교하여 바꿈
                if board[nx][ny][-1] > gun[i]:
                    if gun[i] ==0:
                        gun[i] = board[nx][ny][-1]
                        board[nx][ny] = board[nx][ny][:-1]
                    else:
                        gun[i], board[nx][ny][-1] = board[nx][ny][-1], gun[i]
            del player_loc[(x,y)]
            player_loc[(nx,ny)] = i
